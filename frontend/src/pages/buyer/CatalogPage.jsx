    ...
    <div className="catalog-page">
      ...
      <div className="filter-bar">
        ...
        <input
          ...
          style={{ minWidth: 240 }}
        />
        <select value={genreId} onChange={(e) => setFilter('genre_id', e.target.value)}>
          <option value="">Все жанры</option>
          {genres.map((g) => <option key={g.id} value={g.id}>{g.name}</option>)}
        </select>
        <select value={authorId} onChange={(e) => setFilter('author_id', e.target.value)}>
        ...
        <label className="catalog-page__toggle">
          <input
            type="checkbox"
            checked={outOfStock}
            onChange={(e) => setFilter('out_of_stock', e.target.checked ? 'true' : '')}
          />
          Нет в наличии
        </label>
        {(genreId || authorId || outOfStock || search) && (
          <button className="btn btn--ghost btn--sm" onClick={() => { setSearchParams({}); setSearch(''); }}>
            Сбросить
          </button>
        )}
      </div>
      ...
      {loading ? (
        ...
        <EmptyState icon="📭" title="Ничего не найдено" text="Попробуйте изменить фильтры" />
      ) : (
        <div className="book-grid">
          {filtered.map((book) => (
            <div key={book.id} className="book-card card card--hover" onClick={() => setSelectedBook(book)}>
              <div className="book-card__cover">
                <div className="book-card__cover-inner">
                  <span className="book-card__cover-icon">
                    {book.media_type === 'audio' ? '🎧' : book.media_type === 'ebook' ? '💻' : '📖'}
                  </span>
                </div>
              </div>
              <div className="book-card__body">
                ...
                <div className="book-card__footer">
                  <div className="book-card__price">{Number(book.retail_price).toFixed(2)} ₽</div>
